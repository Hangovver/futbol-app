...
# ⬇️ ÖNEMLİ: Kardeş modüller için göreli import kullan
...
def _derive_booleans(team_id: int, venue: str, n: int = 5):
    ...
    out = {
        "team_ge1": [], "team_ge2": [], "team_ge3": [],
        "tot_ge2": [], "tot_ge3": [], "tot_ge4": [],
        "btts": [], "win": [], "draw": [], "not_lose": [],
        "sample": 0,
    }
    for m in matches:
        hg, ag = _goals(m)
        ...
        if is_home:
            out["win"].append(home_w is True)
            out["draw"].append(home_w is None and (hg == ag))
            out["not_lose"].append((home_w is True) or (hg == ag))
        else:
            out["win"].append(away_w is True)
            out["draw"].append(away_w is None and (hg == ag))
            out["not_lose"].append((away_w is True) or (hg == ag))
    return out
...
def get_predictions_today(min_prob: float = DEFAULT_MIN_PROB) -> List[Dict[str, Any]]:
    ...
    for f in fixtures_today():
        ...
        H = _derive_booleans(hid, "home", 5)
        A = _derive_booleans(aid, "away", 5)

        probs: Dict[str, float] = {}
        probs["HOME_O0.5"] = _ratio(H["team_ge1"])
        probs["HOME_O1.5"] = _ratio(H["team_ge2"])
        probs["HOME_O2.5"] = _ratio(H["team_ge3"])
        probs["AWAY_O0.5"] = _ratio(A["team_ge1"])
        probs["AWAY_O1.5"] = _ratio(A["team_ge2"])
        probs["AWAY_O2.5"] = _ratio(A["team_ge3"])
        probs["O1.5"] = _both_min(H["tot_ge2"], A["tot_ge2"])
        probs["O2.5"] = _both_min(H["tot_ge3"], A["tot_ge3"])
        probs["O3.5"] = _both_min(H["tot_ge4"], A["tot_ge4"])
        probs["BTTS"] = _both_min(H["btts"], A["btts"])
        probs["BTTS_AND_O2.5"] = min(probs["BTTS"], probs["O2.5"])
        probs["MS1"] = _ratio(H["win"])
        probs["MS2"] = _ratio(A["win"])
        probs["MSX"] = _both_min(H["draw"], A["draw"])
        probs["DC_1X"] = _ratio(H["not_lose"])
        probs["DC_X2"] = _ratio(A["not_lose"])
        probs["DC_12"] = min(_ratio(H["win"]), _ratio(A["win"]))
        probs["DNB1"] = _ratio(H["win"])
        probs["DNB2"] = _ratio(A["win"])

        filtered = {}
        for k, v in probs.items():
            if v >= min_prob:
                if k.startswith("U") or "_U" in k:
                    continue
