...
# /backend/static klasörünü servis et
...
def predictions(
    minProb: Optional[float] = Query(None),
...
def match_api(fixture_id: int) -> Dict[str, Any]:
    return match_full_details(fixture_id)


@app.get("/api/team/{team_id}")
def team_api(team_id: int) -> Dict[str, Any]:
    return team_overview(team_id)


@app.post("/api/implied-odds")
def implied_odds(body: Dict[str, Any] = Body(...)) -> Dict[str, Any]:
    selections = body.get("selections") or []
    return calc_implied_for_coupon(selections)
