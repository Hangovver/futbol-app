...
from backend.services import (
    live_fixtures, match_full_details, team_overview, calc_implied_for_coupon,
    standings_for_fixture, comparison_for_fixture, form_for_fixture, live_detail,
    get_comments, add_comment
)

load_dotenv()

app = FastAPI(title="Football Auto API (Full)", version="5.4.0")
...
# --------- YENİ: Puan Durumu / Karşılaştırma / Form / Canlı Detay / Yorumlar ----------

@app.get("/api/standings")
def standings_api(fixtureId: int = Query(...)):
    return standings_for_fixture(fixtureId)

@app.get("/api/comparison")
def comparison_api(fixtureId: int = Query(...), n: int = Query(5)):
    return comparison_for_fixture(fixtureId, n=n)

@app.get("/api/form")
def form_api(fixtureId: int = Query(...), n: int = Query(5)):
    return form_for_fixture(fixtureId, n=n)

@app.get("/api/live-detail/{fixture_id}")
def live_detail_api(fixture_id: int):
    return live_detail(fixture_id)

@app.get("/api/comments")
def get_comments_api(fixtureId: int = Query(...)):
    return get_comments(fixtureId)

@app.post("/api/comments")
def add_comment_api(body: Dict[str, Any] = Body(...)):
    fixture_id = int(body.get("fixtureId"))
    user = body.get("user") or "Anon"
    text = body.get("text") or ""
    return add_comment(fixture_id, user, text)
